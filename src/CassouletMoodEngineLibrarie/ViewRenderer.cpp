...
#include <CassouletMoodEngineLibrarie/ClassicDefs.h>
...
void  ViewRenderer::Init(Map* pMap, Player* pPlayer, int sizeX, int sizeY)
{
	if (m_UseClassicDoomScreenToAngle)
	{
		...
		m_iRenderXSize = sizeX;
		m_iRenderYSize = sizeY;
	}
	m_pMap = pMap;
	m_pPlayer = pPlayer;
...
int ViewRenderer::GetWindowHeight()
{
	return m_iRenderYSize;
}

int ViewRenderer::GetWindowWidth()
{
	return m_iRenderXSize;
}
...
void ViewRenderer::DrawLowerSection(ViewRenderer::SegmentRenderData& RenderData, int iXCurrent, int CurrentFloorStart)
{
	if (RenderData.bDrawLowerSection)
	{
		...
		{
			Wall* _texture = GetSectionTexture(RenderData.pSeg->pLinedef->pRightSidedef->LowerTexture) ;
			_texture->Render(m_pScreenBuffer,m_iBufferPitch,iXCurrent,CurrentFloorStart);
...
Wall* ViewRenderer::GetSectionTexture(const std::string& TextureName)
{
	return AssetsManager::GetInstance()->GetTexture(TextureName);
}
