...
Player::Player(ViewRenderer* pViewRenderer, int iID) : m_pViewRenderer(pViewRenderer), m_iPlayerID(iID), m_FOV(90), m_iRotationSpeed(8), m_iMoveSpeed(4), m_EyeLevel(41)
{
    m_ZPosition = m_EyeLevel;
    m_pWeapon = std::make_unique<Weapon>(Weapon("PISGA0",pViewRenderer->GetWindowWidth(),pViewRenderer->GetWindowHeight()));
}
...
void Player::UpdateInput(const uint8_t* KeyStates,int widthscreen,int heigthscreen)
{
    unsigned short window_center_x = static_cast<unsigned short>(round(0.5f * widthscreen));
    unsigned short window_center_y = static_cast<unsigned short>(round(0.5f * heigthscreen));


    float rotation_horizontal = 90 * (window_center_x - xMouse /widthscreen * m_iRotationSpeed);

    
    m_Angle =window_center_x + rotation_horizontal;

    xMouse = window_center_x;
    yMouse = window_center_y;

    float movementX = 0, movementY = 0; // Vecteur de mouvement
    bool updownIspressed = false;

    if (KeyStates[SDL_SCANCODE_UP])
    {
        movementX += m_Angle.GetCosValue() * m_iMoveSpeed;
        movementY += m_Angle.GetSinValue() *m_iMoveSpeed;
    ...
    float localspeedMove = m_iMoveSpeed;
    if (updownIspressed)
    {
        localspeedMove /= 2;
    }
    ...
    {
        movementX += m_Angle.GetCosValue() - 90 * localspeedMove;
        movementY += m_Angle.GetSinValue() - 90 * localspeedMove;
    }
    ...
    m_XPosition += movementX;
    m_YPosition += movementY;
